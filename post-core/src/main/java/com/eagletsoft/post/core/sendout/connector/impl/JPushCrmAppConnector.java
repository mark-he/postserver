...
@Component
public class JPushCrmAppConnector extends AbstractConnector {
    private static Logger LOG = LoggerFactory.getLogger(JPushCrmAppConnector.class);
    ...
    public void send(SendOutMessage sm) {
        ...
        String message = (String) sm.getContent().get("alert");
        String extras = null;
        if (null != sm.getContent().get("extra")) {
            extras = JsonUtils.writeValue(sm.getContent().get("extra"));
        }
        List<String> reList = new ArrayList<String>();
        reList.add(sm.getReceiver());
        String[] receiver = reList.toArray(new String[] {});

        if (!"dynamic_admin".equals(sm.getSender())) { //“动态”群组不伴随极光推送
